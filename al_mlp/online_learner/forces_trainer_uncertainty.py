...
class ForcesTrainer(BaseTrainer):
    ...
    def predict(
            ...
            if per_image:
                ...
                if results_file is not None:
                    ...
                    _per_image_free_forces_uncertainty = [
                        ...
                        for uncertainty, fixed in zip(
                            per_image_forces_uncertainty, _per_image_fixed
                        )
                    ]
                    _chunk_idx = np.array(
                ...
                predictions["forces_uncertainty"].extend(per_images_forces_uncertainty)
            else:
        ...
        predictions["forces_uncertainty"] = np.array(predictions["forces_uncertainty"])
    ...
    def _forward(self, batch_list):
        # forward pass.
        if self.config["model_attributes"].get("regress_forces", True):
            out_energy, out_forces, forces_uncertainty = self.model(batch_list)
        else:
        ...
        if self.config["model_attributes"].get("regress_forces", True):
            ...
            out["forces_uncertainty"] = forces_uncertainty
