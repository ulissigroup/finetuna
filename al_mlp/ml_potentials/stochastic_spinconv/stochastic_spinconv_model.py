...
from .stochastic_spinconv_utils import conditional_grad, Display
...
@registry.register_model("spinconv_grad11_uncertainty")
class spinconv(BaseModel):
    def __init__(
        ...
        lmax=False,
        force_estimator="direct",
        energy_estimator="energyblock",
        model_ref_number=0,
        display_iter=5000,
        readout="add",
        ...
        self.display_base_name = "/checkpoint/zitnick/ase/spinconv_paper/"
        ...
        if gaussian_learnable == True:
            self.distance_expansion_forces = GaussianSmearingLearnable(
                0.0, cutoff, num_basis_functions
            )
        else:
        ...
        if energy_estimator == "mlpnode" or energy_estimator == "mlpedge":
        ...
        if energy_estimator == "energyblock":
            self.energy_output_block = EnergyOutputBlock(
                hidden_channels,
                mid_hidden_channels,
                embedding_size,
                self.sphere_size_lat,
        ...
        if force_estimator == "direct" or force_estimator == "random":
            self.force_output_blocks = ModuleList()
            for _ in range(self.num_random_rotations):
                ...
                self.force_output_blocks.append(force_output_block)

    @conditional_grad(torch.enable_grad())
    def forward(self, data):
        self.device = data.pos.device
        self.num_atoms = len(data.batch)
        self.batch_size = len(torch.unique(data.batch))
        ...
        # if self.counter % 10000 == self.display_iter:
        #    self._display_helper()
    ...
    def _forward_helper(self, data, edge_index, edge_distance, edge_distance_vec):
        ...
        ###############################################################
        assert self.force_estimator in ["direct", "random", "grad"]
        assert self.energy_estimator in ["mlpnode", "mlpedge", "energyblock"]

        if self.energy_estimator == "mlpnode":
        ...
        if self.energy_estimator == "mlpedge":
            energy = self.energyembeddingblock(x, source_element, target_element)
            energy = scatter(energy, edge_index[1], dim=0, dim_size=data.num_nodes) / (
                self.max_num_neighbors / 2.0 + 1.0
            )
            energy = scatter(energy, data.batch, dim=0)
        if self.energy_estimator == "energyblock":
            energy = self.energy_output_block(
                x,
                x_dist,
                source_element,
                target_element,
        ...
        if self.regress_forces:
            ...
            if self.force_estimator == "direct":
                forces = self._compute_EF_directly(
                    x,
            ...
            if self.force_estimator == "random":
                forces, forces_uncertainty = self._compute_EF_random_rotations(
                    x,
    ...
    def _compute_EF_random_rotations(
        ...
        forces_uncertainty = []
        for rot_index, block in enumerate(self.force_output_blocks):
            ...
            forces_uncertainty.append(forces_perturb)

        forces_uncertainty = torch.stack(forces_uncertainty)
        forces_uncertainty = torch.std(forces_uncertainty, dim=0)
        forces = forces / (num_random_rotations)

        return forces, forces_uncertainty

    def _compute_EF_directly(
        self, x, target_element, edge_index, edge_distance_vec, batch
    ):
        device = x.device
        ...
        (
            ...
            proj_nodes_delta_x,
            proj_nodes_src_index_x,
        ) = self._project2D_nodes_init(rot_mat_x, edge_index, edge_distance_vec)
        (
            proj_nodes_index_y,
            proj_nodes_delta_y,
            proj_nodes_src_index_y,
        ) = self._project2D_nodes_init(rot_mat_y, edge_index, edge_distance_vec)
        (
            proj_nodes_index_z,
            proj_nodes_delta_z,
            proj_nodes_src_index_z,
        ) = self._project2D_nodes_init(rot_mat_z, edge_index, edge_distance_vec)
        ...
        forces = torch.cat([force_x[:, 0:1], force_y[:, 0:1], force_z[:, 0:1]], dim=1)
    ...
    def _filter_edges(
        ...
        # print('s {} {} {} {}'.format(len(torch.masked_select(edge_count_start, edge_count_start.eq(1))), len(torch.masked_select(edge_count_start, edge_count_start.eq(2))),
        #    len(torch.masked_select(edge_count_start, edge_count_start.eq(3))), len(torch.masked_select(edge_count_start, edge_count_start.eq(4)))))
    ...
    def _project2D_edges_init(self, rot_mat, edge_index, edge_distance_vec):
        ...
        # keeping code here, but using the below hurts accuracy
        # remove pairs of edges that create a cycle
        # target_node = edge_index[1][target_edge]
        # source_node = edge_index[0][source_edge]
        # cycle_mask = target_node.ne(source_node)
        # source_edge = torch.masked_select(source_edge, cycle_mask)
        # target_edge = torch.masked_select(target_edge, cycle_mask)
    ...
    def _project2D_init(self, source_edge, target_edge, rot_mat, edge_distance_vec):
        ...
        # Compute mean angle weighted by the distance of the edge (Guassian weighted)
        dist_var = 2.0 * 2.0
        distance_weight = torch.exp(-edge_distance_sqr / (2.0 * dist_var))
        distance_weight = distance_weight[source_edge]
        distance_weight = distance_weight.view(-1, 1).repeat(1, 2).view(-1)

        target_mean_angle = torch.zeros(len(target_edge) * 2, device=device)
        index = torch.cat(
            [
                (2 * target_edge).view(-1, 1),
                (2 * target_edge + 1).view(-1, 1),
            ],
            dim=1,
        ).view(-1)
        longitude_angle = source_edge_offset_rot[:, 1:3].contiguous().view(-1)

        target_mean_angle.index_add_(0, index, distance_weight * longitude_angle)
        target_mean_angle = target_mean_angle.view(-1, 2)
        target_mean_angle = torch.atan2(
            target_mean_angle[:, 0], target_mean_angle[:, 1]
        )
        target_mean_angle = target_mean_angle[target_edge]

        source_edge_X = torch.atan2(
            source_edge_offset_rot[:, 1], source_edge_offset_rot[:, 2]
        ).view(-1)
        # subtract off the mean angle
        source_edge_X = source_edge_X - target_mean_angle
    ...
    def _display_helper(self):
        # Sorry a bit messy, random code to display learned weights

        # display distance function
        dist_x = self.cutoff * torch.arange(200, device=self.device) / 200.0
        dist_x = self.distance_expansion_forces(dist_x)
        dist_x = self.dist_block.fc1(dist_x)
        Display.display_model_fc_weights(
            dist_x,
            self.display_base_name + "dist_" + str(self.model_ref_number),
        )

        if self.scale_distances == True:
            dist_weights = self.dist_block.dist_scalar.weight.data.view(
                self.max_num_elements, self.max_num_elements
            )
            Display.display_model_fc_weights(
                dist_weights,
                self.display_base_name + "dist_scalar_" + str(self.model_ref_number),
            )
            dist_weights = self.dist_block.dist_offset.weight.data.view(
                self.max_num_elements, self.max_num_elements
            )
            Display.display_model_fc_weights(
                dist_weights,
                self.display_base_name + "dist_offset_" + str(self.model_ref_number),
            )

        # display final conv filters
        """
        conv_weights = self.output_block.conv1.weight.data
        ...
        Display.display_model_conv_weights(
            conv_weights,
            self.display_base_name
            + "force_conv1_"
            + str(self.model_ref_number),
        )
        """

        # display interaction block params
        for block_index, interaction in enumerate(self.message_blocks):

            Display.display_model_fc_weights(
                interaction.embeddingblock1.fc2.weight.data,
                self.display_base_name
                + "block1_fc2_"
                + str(self.model_ref_number)
                + "_"
            ...
            Display.display_model_fc_weights(
                interaction.embeddingblock1.fc3.weight.data,
                self.display_base_name
                + "block1_fc3_"
                + str(self.model_ref_number)
                + "_"
            ...
            Display.display_model_fc_weights(
                interaction.embeddingblock1.source_embedding.weight.data,
                self.display_base_name
                + "block1_source_embedding_"
                + str(self.model_ref_number)
                + "_"
                + str(block_index + 1),
            )
            Display.display_model_fc_weights(
                interaction.embeddingblock1.target_embedding.weight.data,
                self.display_base_name
                + "block1_target_embedding_"
                + str(self.model_ref_number)
                + "_"
                + str(block_index + 1),
            )
            ...
            Display.display_model_fc_weights(
                interaction.embeddingblock2.fc2.weight.data,
                self.display_base_name
                + "block2_fc2_"
                + str(self.model_ref_number)
                + "_"
            ...
            Display.display_model_fc_weights(
                interaction.embeddingblock2.fc3.weight.data,
                self.display_base_name
                + "block2_fc3_"
                + str(self.model_ref_number)
                + "_"
            ...
            Display.display_model_fc_weights(
                interaction.embeddingblock2.source_embedding.weight.data,
                self.display_base_name
                + "block2_source_embedding_"
                + str(self.model_ref_number)
                + "_"
                + str(block_index + 1),
            )
            Display.display_model_fc_weights(
                interaction.embeddingblock2.target_embedding.weight.data,
                self.display_base_name
                + "block2_target_embedding_"
                + str(self.model_ref_number)
                + "_"
                + str(block_index + 1),
            )

            if self.sphere_message in ["fullconv", "zeroconv"]:
                conv_weights = interaction.spinconvblock.conv1.weight.data
                conv_weights = conv_weights.view(
                    -1,
                    self.hidden_channels,
                    self.sphere_size_lat,
...
class EnergyOutputBlock(torch.nn.Module):
    def __init__(
        self,
        in_hidden_channels,
        mid_hidden_channels,
        ...
        super(EnergyOutputBlock, self).__init__()
        self.in_hidden_channels = in_hidden_channels
        self.out_hidden_channels = 1
        self.act = act
        self.lmax = lmax
        self.embedding_size = embedding_size
        self.mid_hidden_channels = mid_hidden_channels
...
class SpinConvBlock(torch.nn.Module):
    def __init__(
        ...
        elif self.sphere_message == "fullconv":
        ...
        elif self.sphere_message == "zeroconv":
            padding = 0
            self.conv1 = nn.Conv1d(
                self.in_hidden_channels * self.sphere_size_lat,
                self.mid_hidden_channels,
        ...
        elif self.sphere_message == "flatten":
            self.mlp = nn.Linear(
                self.in_hidden_channels * self.sphere_size_lat * self.sphere_size_long,
                self.mid_hidden_channels,
            )
    ...
    def forward(self, x, out_size, proj_index, proj_delta, proj_src_index):
        ...
        if self.sphere_message == "spharm":
            x = self.sph_froms2grid(x)
            x = x.reshape(x.shape[0], -1)
            x = self.mlp(x)
            x = self.act(x)
        elif self.sphere_message == "rotspharmwd":
            ...
            x = sph_harm_calc
        elif self.sphere_message == "rotspharmroll":
            sph_harm_calc = torch.zeros(
                ((x.shape[0], self.mid_hidden_channels)), device=x.device
            )
            for i in range(self.sphere_size_long):
                sph_harm_calc += self.act(
                    self.mlp(self.sph_froms2grid(x).reshape(x.shape[0], -1))
                )
                x = torch.roll(x, shifts=(0, 1), dims=(2, 3))
            x = sph_harm_calc
        elif self.sphere_message in ["fullconv", "zeroconv"]:
            x = x.view(
                -1,
                self.in_hidden_channels * self.sphere_size_lat,
                self.sphere_size_long,
            ...
            x = x.view(out_size, -1)
        elif self.sphere_message in ["flatten"]:
            x = x.reshape(x.shape[0], -1)
            x = self.mlp(x)
            x = self.act(x)
...
class ProjectLatLongSphere(torch.nn.Module):
    ...
    def forward(self, x, length, index, delta, source_edge_index):
        ...
        # proj_sum = torch.sum(torch.abs(x_proj.view(-1, self.sphere_size_lat, self.sphere_size_long)), dim = 0)
        # proj_sum_atom = torch.sum(torch.abs(x_proj.view(length, hidden_channels, self.sphere_size_lat, self.sphere_size_long)), dim = 1)

        # Display.display_model_fc_weights(proj_sum, "/checkpoint/zitnick/ase/" + 'sphere_avg_a')
        # Display.display_model_fc_weights(proj_sum_atom[0], "/checkpoint/zitnick/ase/" + 'sphere_avg_1a')
...
class GaussianSmearingLearnable(torch.nn.Module):
    def __init__(self, start=-5.0, stop=5.0, num_gaussians=50):
        super(GaussianSmearingLearnable, self).__init__()
        self.offsets = nn.Parameter(
            torch.linspace(start, stop, num_gaussians), requires_grad=True
        )
        self.standarddev = nn.Parameter(
            torch.zeros(num_gaussians) + 1.0, requires_grad=True
        )
    ...
    def forward(self, dist):
        dist = dist.view(-1, 1) - self.offsets.view(1, -1)
...
class SineBasis(torch.nn.Module):
    def __init__(self, num_freq, max_freq):
        super(SineBasis, self).__init__()
        self.num_freq = num_freq
        self.max_freq = max_freq

    def forward(self, x):
        device = x.device
        freq = torch.arange(self.num_freq, device=device).float()
        freq = torch.pow(torch.ones_like(freq) * 1.1, freq)
        freq = (math.pi * freq) / (2.0 * self.max_freq)
        # print('{}'.format(6.0*freq))

        x = x.view(-1, 1).expand(-1, self.num_freq)
        x = torch.sin(x * freq)
