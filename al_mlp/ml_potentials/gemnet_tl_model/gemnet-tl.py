...
@registry.register_model("gemnet_t_tl")
class GemNetT(torch.nn.Module):
    ...
    def generate_interaction_graph(self, data):
        ...
        if self.use_pbc:
            ...
            j, i = edge_index
            D_st = (data.pos[i] - data.pos[j]).pow(2).sum(dim=-1).sqrt()
            V_st = (data.pos[j] - data.pos[i]) / D_st[:, None]
    ...
    def forward(self, data):
        ...
        if self.regress_forces or self.regress_position:
